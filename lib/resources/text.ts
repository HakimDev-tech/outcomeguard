...
function validateTitle(title: string): string {
  ...
  if (normalized.length > 500) {
    throw resourceError("Text resource title is too long.", { maxLength: 500 });
  }
...
function validateUrl(url?: string): string | undefined {
  ...
  try {
    ...
    throw resourceError("Text resource URL is invalid.", { url });
  }
}
...
export function parseTextResource(
  ...
  if (content.length > maxCharacters) {
    throw resourceError("Text resource exceeds the maximum allowed size.", {
